...
async function sendOrderConfirmation(order) {
  console.log("[sendOrderConfirmation] inicio, buyerEmail:", order?.buyer?.email, "EMAIL_USER:", !!process.env.EMAIL_USER);
  if (!process.env.EMAIL_USER) return;

  const buyerEmail = order?.buyer?.email;
  console.log("[sendOrderConfirmation] buyerEmail:", buyerEmail);
...
console.log("[email] intentando enviar a:", buyerEmail);
try {
  ...
  console.log("[email cliente] ENVIADO OK:", info.messageId);
} catch(e) {
  ...
  if (ADMIN_EMAIL) {
    ...
    await transporter.sendMail({
      ...
      html,
    }).catch(e => console.warn("[email admin nuevo pedido]", e));
  }
}
...
async function sendPaymentConfirmed(order) {
  ...
  if (buyerEmail) {
    ...
    await transporter.sendMail({
      ...
      html,
    }).catch(e => console.warn("[email cliente pago confirmado]", e));
  }
  ...
  if (ADMIN_EMAIL) {
    ...
    await transporter.sendMail({
      ...
      html,
    }).catch(e => console.warn("[email admin pago confirmado]", e));
  }
}
